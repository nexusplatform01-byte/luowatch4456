...
import { Eye, EyeOff, Mail, Lock, User, Film, Music, Clapperboard, Phone, X } from "lucide-react";
...
const roles = [
  { id: "viewer", label: "Viewer", description: "Watch movies, music & more", icon: User, needsPhone: false },
];

const AuthModal = () => {
  const { showAuthModal, setShowAuthModal, authModalTab, setAuthModalTab, login, register, loginWithGoogle, presetRole } = useAuth();
  ...
  const selectedRole = presetRole || "viewer";
  const [phone, setPhone] = useState("");

  const needsPhone = ["vj", "musician", "tiktoker"].includes(selectedRole);
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (needsPhone && !phone) {
      toast({ title: "Error", description: "Phone number is required for this role", variant: "destructive" });
      return;
    }
    setLoading(true);
    ...
    setLoading(false);
    if (result.success) {
      toast({ title: "Account created!", description: "Welcome to LUO WATCH" });
    } else {
      toast({ title: "Error", description: result.error, variant: "destructive" });
    }
  };
          ...
          {authModalTab === "login" ? (
            <form className="space-y-3" onSubmit={handleLogin}>
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-secondary text-foreground text-xs pl-8 pr-3 py-2 rounded border border-border focus:outline-none focus:ring-1 focus:ring-primary"
                  />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-secondary text-foreground text-xs pl-8 pr-8 py-2 rounded border border-border focus:outline-none focus:ring-1 focus:ring-primary"
                  />
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-primary text-primary-foreground py-2 rounded text-xs font-bold hover:bg-primary/90 transition-colors disabled:opacity-50"
              >
              ...
              <button
                ...
                disabled={loading}
                className="w-full bg-secondary text-foreground py-2 rounded text-xs font-semibold hover:bg-secondary/80 transition-colors flex items-center justify-center gap-2 border border-border disabled:opacity-50"
              >
            ...
            <form className="space-y-3" onSubmit={handleRegister}>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-foreground text-[11px] font-semibold mb-1 block">First Name</label>
                  <input type="text" placeholder="First name" value={firstName} onChange={(e) => setFirstName(e.target.value)} className="w-full bg-secondary text-foreground text-xs px-3 py-2 rounded border border-border focus:outline-none focus:ring-1 focus:ring-primary" />
                </div>
                <div>
                  <label className="text-foreground text-[11px] font-semibold mb-1 block">Last Name</label>
                  <input type="text" placeholder="Last name" value={lastName} onChange={(e) => setLastName(e.target.value)} className="w-full bg-secondary text-foreground text-xs px-3 py-2 rounded border border-border focus:outline-none focus:ring-1 focus:ring-primary" />
                </div>
              </div>
              <div>
                <label className="text-foreground text-[11px] font-semibold mb-1 block">Email</label>
